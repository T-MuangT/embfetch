...
4. Add `src/sysinfo_common.c src/sysinfo_zephyr.c` into `CMakeLists.txt`'s `target_sources()`.
...
3. Add entry and register `embfetch` command into your main or shell source file.
4. Add `src/sysinfo_common.c src/sysinfo_threadx.c` into `CMakeLists.txt`'s `add_executable()`.
...
2. Copy `sysinfo_common.c` from `src`, `embed_sysinfo.h` from `src/include`, `sysinfo_freertos.c` from `src/rtos/freertos`, and `logo.h` from `src/rtos/freertos/include` into your kernel's `src` directory.
3. Add entry and register `embfetch` command into your main or shell source file.
4. Add `sysinfo_common.c` and `sysinfo_freertos.c` into `SRCS` in Makefile.
...
2. Copy `sysinfo_common.c` from `src`, `embed_sysinfo.h` from `src/include`, `sysinfo_esp-idf.c` from `src/rtos/esp-idf`, and `logo.h` from `src/rtos/esp-idf/include` into your kernel's `main` directory.
3. Add entry and register `embfetch` command into your main or shell source file.
4. Add `sysinfo_common.c sysinfo_esp-idf.c` into your `main/CMakeLists.txt`'s `SRCS`.
...
3. Add `sysinfo_common.c` from `src`, `embed_sysinfo.h` from `src/include`, `sysinfo_keil-rtx.c` from `src/rtos/keil-rtx`, and `logo.h` from `src/rtos/keil-rtx/include` into your Keil RTX5 project.
